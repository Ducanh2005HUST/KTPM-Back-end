document.addEventListener('DOMContentLoaded', function() {
    ...
    const persons = [
        { id: 101, full_name: 'Nguyễn Văn A', id_number: '012345678', household_code: 'HK-001' },
        { id: 102, full_name: 'Trần Thị B', id_number: '098765432', household_code: 'HK-002' },
        { id: 103, full_name: 'Lê Văn C', id_number: '111222333', household_code: 'HK-003' },
        { id: 104, full_name: 'Phạm Thị D', id_number: '444555666', household_code: 'HK-001' }
    ];
    ...
    function displayHouseholdResults(results) {
        ...
        if (results.length === 0) {
            resultsBody.innerHTML = '<div class="no-results">Không tìm thấy hộ khẩu nào</div>';
        } else {
    ...
    function displayPersonResults(results) {
        ...
        if (results.length === 0) {
            resultsBody.innerHTML = '<div class="no-results">Không tìm thấy nhân khẩu nào</div>';
        } else {
    ...
    document.addEventListener('click', function(event) {
        if (!householdSearchInput.contains(event.target) && !householdResults.contains(event.target)) {
            householdResults.style.display = 'none';
        }
        if (!personSearchInput.contains(event.target) && !personResults.contains(event.target)) {
            personResults.style.display = 'none';
        }
    });
